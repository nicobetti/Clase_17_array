...
const sistemaDePeliculasConMetodosDeArrays = {
	...
	filtrarPorRating: function(ratingMinimo) {
		return this.peliculas.filter((peli) => peli.calificacionIMDB >= ratingMinimo);
	},
	agregarGenero: function(generoBuscado, generoAAgregar) {
		this.peliculas.filter((peli) => peli.genero.includes(generoBuscado));
		for (const i of this.peliculas) {
			i.genero.push(generoAAgregar);
		}
		return this.peliculas;
	},
	promedioDeDuracion: function() {
		const aDuraciones = [];
		for (const iterator of this.peliculas) {
			aDuraciones.push(iterator['duracionEnMinutos']);
		}
		duracionTotal = aDuraciones.reduce((previousValue, currentValue) => previousValue + currentValue);
		return duracionTotal / aDuraciones.length;
	}
};
...
console.log(sistemaDePeliculasConMetodosDeArrays.agregarGenero('Accion', 'Aventura'));
